...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def PPO_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        _loss.backward()
        # The paper's common hyperparameter table specifies policy grad clipping at 0.5.
        # This is required for the reported 1e-2 policy learning rate to remain stable.
    ...
    def Advantage_ActorUpdate(_obs, _act, _adv, _outputs_old):
        _outputs = actor_critic.forward_pi(_obs)
        ...
        with torch.no_grad():
            ...
            # _png_adv = torch.mv( torch.inverse(HHT + algo_config.cg_damping * torch.eye(num_sa, device=device)), _adv)
    ...
    if algo in {'ppo'}: 
        ...
        update_actor = Advantage_ActorUpdate
