...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if algo_config.optimizer == 'adam':
        ...
        pi_optimizer = SGD(
            params_pi,
            lr=algo_config.lr_pi,
            momentum=float(getattr(algo_config, 'pi_momentum', 0.0)),
        )
    ...
    if critic_optimizer_name == 'adam':
        ...
        v_optimizer = SGD(
            params_v,
            lr=algo_config.lr_v,
            momentum=float(getattr(algo_config, 'v_momentum', 0.0)),
        )
    else:
        raise ValueError(f"Unsupported critic_optimizer: {critic_optimizer_name}")
    print(
        f"Optimizer control: pi_momentum={getattr(algo_config, 'pi_momentum', 0.0)} "
        f"v_momentum={getattr(algo_config, 'v_momentum', 0.0)} "
        f"adaptive_kl={algo_config.use_kl_adaptive_lr} "
        f"kl_target={getattr(algo_config, 'kl_target', 0.008)} "
        f"kl_bounds=[{getattr(algo_config, 'kl_lr_min', 1e-4)}, {getattr(algo_config, 'kl_lr_max', 5e-2)}]",
        flush=True,
    )
    ...
    if actor_clip_mode not in {'l2', 'fisher_norm', 'fisher_l2', 'fvp_fisher'}:
    ...
    print(
        ...
        "fisher_norm metric=H^T D_ratio H / n + damping I",
        flush=True,
    )

    def clip_actor_score_fisher_norm_(score_rows, ratio, damping, max_norm):
        """Clip with a PSD score-Fisher metric built from the anchor rows."""
        with torch.no_grad():
            flat_grad = torch.cat([
                (p.grad if p.grad is not None else torch.zeros_like(p)).contiguous().view(-1)
                for p in params_pi
            ])
            score_rows = score_rows.to(device=flat_grad.device, dtype=flat_grad.dtype)
            ratio = ratio.detach().to(device=flat_grad.device, dtype=flat_grad.dtype)
            response = torch.mv(score_rows, flat_grad)
            fisher_norm_sq = (
                torch.dot(ratio, response.square()) / float(score_rows.shape[0])
                + float(damping) * torch.dot(flat_grad, flat_grad)
            ).clamp_min(0.0)
            fisher_norm = fisher_norm_sq.sqrt()
            clip_coef = torch.clamp(
                torch.as_tensor(max_norm, device=flat_grad.device, dtype=flat_grad.dtype)
                / (fisher_norm + 1e-12),
                max=1.0,
            )
            for p in params_pi:
            ...
            return flat_grad.norm(), fisher_norm, fisher_norm * clip_coef, clip_coef
    ...
    def RAT_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        if use_grouped_rho:
            ...
            # Large minibatches cannot materialize the full m x p score matrix.
            # The anchor solve needs only 255 explicit rows; the non-anchor
            # contribution is accumulated later as one weighted score gradient.
            ft_per_sample_grads_full = None
            ft_per_sample_grads = ft_compute_sample_grad(
                dict_params, dict_buffers, _actor_obs, _actor_act
            )
        else:
            ft_per_sample_grads_full = None
        ...
        actor_l2_norm_post = torch.tensor(float('nan'), device=_obs.device)
        ...
        actor_fisher_norm_pre = torch.tensor(float('nan'), device=_obs.device)
        actor_fisher_norm_post = torch.tensor(float('nan'), device=_obs.device)
        actor_fisher_clip_coef = torch.tensor(1.0, device=_obs.device)
        actor_l2_clip_coef = torch.tensor(1.0, device=_obs.device)
        ...
        if use_grouped_rho:
            ...
            _loss_pi = (-_ratio.detach() * _logp * _adv.detach()).mean()
            pi_optimizer.zero_grad()
            full_m = int(_obs.shape[0])
            basis_idx = actor_curvature_inds
            basis_mask = torch.zeros(full_m, device=_obs.device, dtype=torch.bool)
            basis_mask[basis_idx] = True
            nonbasis_idx = torch.arange(full_m, device=_obs.device)[~basis_mask]
            target_full = _adv.detach().to(dtype=H.dtype)
            ratio_full = _ratio.detach().to(dtype=H.dtype)
            target_s = target_full.index_select(0, basis_idx)
            ratio_s = ratio_full.index_select(0, basis_idx)

            if g_k is not None:
                raise NotImplementedError(
                    "streaming large-batch free-rho currently requires Kaczmarz=false"
                )
            if actor_free_rho_mode != 'anchor':
                raise NotImplementedError(
                    "streaming large-batch free-rho currently supports anchor mode only"
                )

            if nonbasis_idx.numel() > 0:
                target_n = target_full.index_select(0, nonbasis_idx)
                ratio_n = ratio_full.index_select(0, nonbasis_idx)
                weighted_nonanchor_logp = torch.dot(
                    _logp.index_select(0, nonbasis_idx),
                    (ratio_n * target_n).detach().to(dtype=_logp.dtype),
                )
                nonanchor_score_grads = torch.autograd.grad(
                    weighted_nonanchor_logp,
                    params_pi,
                    retain_graph=True,
                )
                g_n_b_n = make_flat(nonanchor_score_grads).detach().to(dtype=H.dtype)
            else:
                target_n = target_full.new_zeros((0,))
                ratio_n = ratio_full.new_zeros((0,))
                g_n_b_n = torch.zeros(H.shape[1], device=H.device, dtype=H.dtype)

            _loss_pi.backward(retain_graph=actor_clip_mode == 'fvp_fisher')
            with torch.no_grad():
                damping = float(algo_config.cg_damping)
                if damping <= 0.0:
                    raise ValueError("actor_subsample_free_rho requires positive cg_damping")
                if actor_curvature_inds is None:
                ...
                if actor_free_rho_mode == 'anchor':
                    ...
                    if not torch.isfinite(rho):
                        raise FloatingPointError("non-finite Energy-free255+1 rho")
            ...
            pi_optimizer.zero_grad()
            _loss = _loss_pi - algo_config.ent_coef * _entropy
            _loss.backward(retain_graph=actor_clip_mode == 'fvp_fisher')
        ...
        if actor_clip_mode == 'fvp_fisher':
            ...
            if not torch.isfinite(actor_fvp_quadratic_pre):
                raise FloatingPointError("non-finite actor FVP quadratic")
            ...
            if not torch.isfinite(actor_clip_coef) or not torch.isfinite(actor_fvp_quadratic_post):
                raise FloatingPointError("non-finite actor FVP clip diagnostics")
            grad_norm = actor_fvp_quadratic_pre
        elif actor_clip_mode in {'fisher_norm', 'fisher_l2'}:
            grad_norm, actor_fisher_norm_pre, actor_fisher_norm_post, actor_fisher_clip_coef = clip_actor_score_fisher_norm_(
                H,
                _actor_ratio,
                algo_config.cg_damping,
                algo_config.max_grad_norm,
            )
            actor_clip_coef = actor_fisher_clip_coef
            if actor_clip_mode == 'fisher_l2':
                l2_pre = torch.nn.utils.clip_grad_norm_(params_pi, algo_config.max_grad_norm)
                actor_l2_norm_pre = l2_pre
                actor_l2_clip_coef = torch.clamp(
                    torch.as_tensor(algo_config.max_grad_norm, device=l2_pre.device, dtype=l2_pre.dtype) / l2_pre,
                    max=1.0,
                )
                actor_l2_norm_post = l2_pre * actor_l2_clip_coef
                actor_clip_coef = actor_fisher_clip_coef * actor_l2_clip_coef
        elif actor_clip_mode == 'l2':
            grad_norm = torch.nn.utils.clip_grad_norm_(params_pi, algo_config.max_grad_norm)
        pi_optimizer.step()
        if pi_scheduler is not None:
            pi_scheduler.step()
        ...
        # Useful extra info
        with torch.no_grad():
            if not torch.isfinite(_real_kl):
            ...
            pi_info = dict(kl=_real_kl.item(), curr_lr=pi_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_l2_norm_post=actor_l2_norm_post.item(),
                           actor_fvp_quadratic_pre=actor_fvp_quadratic_pre.item(),
                           actor_fvp_quadratic_post=actor_fvp_quadratic_post.item(),
                           actor_fisher_norm_pre=actor_fisher_norm_pre.item(),
                           actor_fisher_norm_post=actor_fisher_norm_post.item(),
                           actor_fisher_clip_coef=actor_fisher_clip_coef.item(),
                           actor_l2_clip_coef=actor_l2_clip_coef.item(),
    ...
    for epoch in tepochs:
        ...
        if algo_config.use_kl_adaptive_lr:
            curr_kl = pi_info['kl']
            kl_target = float(getattr(algo_config, 'kl_target', 0.008))
            kl_factor = float(getattr(algo_config, 'kl_lr_factor', 1.5))
            kl_min_lr = float(getattr(algo_config, 'kl_lr_min', 1e-4))
            kl_max_lr = float(getattr(algo_config, 'kl_lr_max', 5e-2))
            if curr_kl > kl_target * 2:
                pi_optimizer.param_groups[0]['lr'] = max(pi_optimizer.param_groups[0]['lr'] / kl_factor, kl_min_lr)
            elif curr_kl < kl_target / 2:
                pi_optimizer.param_groups[0]['lr'] = min(pi_optimizer.param_groups[0]['lr'] * kl_factor, kl_max_lr)
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            elif pi_info.get('actor_clip_mode') in {'fisher_norm', 'fisher_l2'}:
                logger.logkv("actor_fisher_norm_pre", pi_info['actor_fisher_norm_pre'])
                logger.logkv("actor_fisher_norm_post", pi_info['actor_fisher_norm_post'])
                logger.logkv("actor_fisher_clip_coef", pi_info['actor_fisher_clip_coef'])
                if pi_info.get('actor_clip_mode') == 'fisher_l2':
                    logger.logkv("actor_l2_norm_pre", pi_info['actor_l2_norm_pre'])
                    logger.logkv("actor_l2_norm_post", pi_info['actor_l2_norm_post'])
                    logger.logkv("actor_l2_clip_coef", pi_info['actor_l2_clip_coef'])
                logger.logkv("actor_clip_coef", pi_info['actor_clip_coef'])
        ...
        if writer is not None:
            ...
            if pi_info.get('actor_clip_mode') == 'fvp_fisher':
                ...
                writer.add_scalar("actor_fisher_norm_pre", pi_info['actor_fisher_norm_pre'], epoch)
                writer.add_scalar("actor_fisher_norm_post", pi_info['actor_fisher_norm_post'], epoch)
                writer.add_scalar("actor_fisher_clip_coef", pi_info['actor_fisher_clip_coef'], epoch)
                if pi_info.get('actor_clip_mode') == 'fisher_l2':
                    writer.add_scalar("actor_l2_norm_pre", pi_info['actor_l2_norm_pre'], epoch)
                    writer.add_scalar("actor_l2_norm_post", pi_info['actor_l2_norm_post'], epoch)
                    writer.add_scalar("actor_l2_clip_coef", pi_info['actor_l2_clip_coef'], epoch)
                writer.add_scalar("actor_clip_coef", pi_info['actor_clip_coef'], epoch)
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        actor_rho_mode = str(getattr(algo_config, 'actor_subsample_free_rho', 'none') or 'none')
        actor_clip_mode = str(getattr(algo_config, 'actor_clip_mode', 'l2') or 'l2')
        print(
            "LARGEBS_ENERGYFREE255P1_CONFIG "
            f"actor_anchor_samples={actor_sub} actor_free_rho_mode={actor_rho_mode} "
            f"actor_dof={actor_sub + int(actor_rho_mode == 'anchor')} "
            f"critic_curvature_samples={critic_sub} critic_update={algo_config.critic_update} "
            f"clip_mode={actor_clip_mode} damping={algo_config.cg_damping} "
            f"kaczmarz={algo_config.is_karzmarz} normalization={algo_config.fisher_kernel_normalization} "
            f"rollout={env_config.num_envs * env_config.nsteps} minibatches={algo_config.pi_minibatches} "
            f"epochs={algo_config.pi_epochs}",
            flush=True,
        )
    ...
    elif nets_config.type == 'transformer':
        kwargs = {
            'device': device,
            'env_name': env_config.env_name,
            'nets_config': nets_config,
        }
        fn_neural_nets, preprocess = build_transformer(obs_space, **kwargs)
        obs_shape = obs_space.shape
...
def main():
    ...
    parser.add_argument('--pi_momentum', type=float, default=None)
    parser.add_argument('--v_momentum', type=float, default=None)
    parser.add_argument('--kl_target', type=float, default=None)
    parser.add_argument('--kl_lr_min', type=float, default=None)
    parser.add_argument('--kl_lr_max', type=float, default=None)
    parser.add_argument('--kl_lr_factor', type=float, default=None)
    parser.add_argument('--actor_clip_mode', type=str, default=None,
                        choices=['l2', 'fisher_norm', 'fisher_l2', 'fvp_fisher'])
    ...
    if args.pi_momentum is not None:
        algo_config.pi_momentum = args.pi_momentum
    if args.kl_target is not None:
        algo_config.kl_target = args.kl_target
    if args.kl_lr_min is not None:
        algo_config.kl_lr_min = args.kl_lr_min
    if args.kl_lr_max is not None:
        algo_config.kl_lr_max = args.kl_lr_max
    if args.kl_lr_factor is not None:
        algo_config.kl_lr_factor = args.kl_lr_factor

    if args.v_momentum is not None:
        algo_config.v_momentum = args.v_momentum
