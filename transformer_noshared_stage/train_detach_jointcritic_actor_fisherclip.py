...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if actor_clip_mode not in {'l2', 'fisher_norm', 'fisher_l2'}:
        raise ValueError(f"Unsupported actor_clip_mode: {actor_clip_mode}")
    print(
        f"Actor clip mode: {actor_clip_mode}; max_norm={algo_config.max_grad_norm}; "
        "fisher_norm metric=H^T D_ratio H / n + damping I",
        flush=True,
    )

    def clip_actor_fisher_norm_(score_rows, ratio, damping, max_norm):
        """Clip the final actor gradient in the damped empirical-Fisher norm.

        This is applied after the transformed RAT loss has been backpropagated.
        It replaces actor Euclidean clipping; it does not touch critic gradients.
        """
        with torch.no_grad():
            flat_grad = torch.cat([
            ...
            l2_norm = flat_grad.norm()
            return l2_norm, fisher_norm, fisher_norm * clip_coef, clip_coef
    ...
    def RAT_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        if actor_clip_mode in {'fisher_norm', 'fisher_l2'}:
            if actor_curvature_inds is not None or use_grouped_rho or use_free_rho or effective_full_gradient_rhs:
                raise ValueError("actor_clip_mode=fisher_norm currently requires full-batch Exact EF")
            if str(getattr(algo_config, 'fisher_kernel', 'exact')).lower() != 'exact':
                raise ValueError("actor_clip_mode=fisher_norm requires fisher_kernel=exact")
            if str(getattr(algo_config, 'fisher_kernel_normalization', 'none')).lower() != 'none':
                raise ValueError("actor_clip_mode=fisher_norm requires fisher_kernel_normalization=none")
            grad_norm, actor_fisher_norm_pre, actor_fisher_norm_post, actor_fisher_clip_coef = clip_actor_fisher_norm_(
                H,
                _actor_ratio,
                algo_config.cg_damping,
    ...
    for epoch in tepochs:
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if pi_info.get('actor_clip_mode') in {'fisher_norm', 'fisher_l2'}:
...
def main():
    ...
    parser.add_argument('--actor_clip_mode', type=str, default=None,
                        choices=['l2', 'fisher_norm', 'fisher_l2'])
