  ...
  for env_name in "${ENVS[@]}"; do
    for seed in 0 1 2 3 4; do
      for method in "${METHODS[@]}"; do
        printf "%s\t%s\t%s\t%s\n" "$((i % NUM_GPUS))" "$method" "$env_name" "$seed"
        i=$((i + 1))
      done
    done
  done
...
if [[ "$TASK_LIMIT" -gt 0 ]]; then
  {
    head -n 1 "$TASKS"
    tail -n +2 "$TASKS" | head -n "$TASK_LIMIT"
  } > "$TASKS.limited"
  mv "$TASKS.limited" "$TASKS"
fi

{
  ...
  echo "launcher=csf3_gpuH4_allmethods_multiworker"
  echo "gpu_workers=$NUM_GPUS"
  echo "max_parallel_per_gpu=$MAX_PARALLEL_PER_GPU"
  echo "task_limit=$TASK_LIMIT"
  echo "parallel_children_cap=$((NUM_GPUS * MAX_PARALLEL_PER_GPU))"
  ...
  echo "launcher_worker_sha256=$(sha256sum "$SOURCE/csf3/csf3_transformer_noshared_gpuH4_allmethods_worker.sh" | awk '{print $1}')"
  echo "launcher_sbatch_sha256=$(sha256sum "$SOURCE/csf3/csf3_transformer_noshared_gpuH4_allmethods.sbatch" | awk '{print $1}')"
...
run_seed() {
  local gpu=$1 method=$2 env_name=$3 seed=$4
  ...
  local rc=$?
  echo "$rc" > "$outdir/rc"
  if [[ $rc -eq 0 ]] && grep -Eiq "out of memory|(^|[^a-z])nan([^a-z]|$)|traceback|linalgerror" "$outdir/stdout.log" "$outdir/stderr.log"; then
...
gpu_worker() {
  ...
  while IFS=$'\t' read -r shard method env_name seed; do
    ...
    (
      rc=0
      run_seed "$gpu" "$method" "$env_name" "$seed" || rc=$?
      rm -rf "$lock_dir"
      exit "$rc"
    ) &
    pids+=("$!")
