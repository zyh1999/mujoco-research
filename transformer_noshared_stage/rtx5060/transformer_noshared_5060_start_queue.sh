...
RUN_ROOT=${RUN_ROOT:-perf_runs/transformer_noshared_5060_all_methods_all7_5seed_10m_$(date +%Y%m%d_%H%M%S)}
...

{
  ...
  echo "total_runs=$((i))"
  echo "per_gpu_concurrency=1"
  echo "started=$(date --iso-8601=seconds)"
} > "$HOST_RUN_ROOT/queue_info.txt"
echo RUNNING > "$HOST_RUN_ROOT/status"

for gpu in 0 1; do
  log="$HOST_RUN_ROOT/workers/gpu${gpu}.nohup.log"
  TOTAL_TIMESTEPS=${TOTAL_TIMESTEPS:-10000000} nohup \
    "$LAUNCH_ROOT/transformer_noshared_5060_worker.sh" "$gpu" "$TASKS_TSV" "$RUN_ROOT" "$SOURCE_NAME" \
    > "$log" 2>&1 &
  echo "$!" > "$HOST_RUN_ROOT/workers/gpu${gpu}.hostpid"
done

printf "%s\n" "$RUN_ROOT" > "$LAUNCH_ROOT/latest_queue_root.txt"
