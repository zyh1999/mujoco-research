...
class ComputeCovG:
    ...
    def linear(g, layer, batch_averaged):
        ...
        if batch_averaged:
            # Transformer Linear layers reuse one parameter matrix at every
            # token. Their inputs are flattened to [batch * locations, dim]
            # for K-FAC-expand, but a mean loss still scales gradients by the
            # original trajectory minibatch, not by batch * locations.
            num_locations = int(getattr(layer, '_kfac_num_locations', 1))
            if num_locations <= 0 or batch_size % num_locations != 0:
                raise ValueError(
                    f'Invalid K-FAC token expansion: rows={batch_size}, '
                    f'locations={num_locations}, layer={layer}'
                )
            loss_batch_size = batch_size // num_locations
            cov_g = g.t() @ (g * loss_batch_size)
        else:
