...
METHODS = [
    "curv256_criticggn_ktrue_l2clip05",
    "curv256_criticggn_ktrue_actor_fvpclip05",
]
SEEDS = range(5)
NUM_SHARDS = 4
...
def command_for(method, python, repo, env, seed, timesteps):
    if method == "curv256_criticggn_ktrue_l2clip05":
        return [
            python,
            "-u",
            str(repo / "train_detach_jointcritic_ktrue_batch262144_20260805.py"),
            "--config",
            "rat_mlp_detjc_curv256_criticggn_ktrue_l2clip05_batch262144_ws92.yaml",
            "--env_name",
            env,
            "--seed",
            str(seed),
    ...
    if method == "curv256_criticggn_ktrue_actor_fvpclip05":
        return [
        python,
        "-u",
        str(repo / "train_detach_jointcritic_actor_fvp_fisherclip_curvsub_ktrue_batch262144_20260805.py"),
        "--config",
        "rat_mlp_detjc_curv256_criticggn_ktrue_actor_fvpclip05_batch262144_ws92.yaml",
        "--env_name",
        env,
        "--seed",
        str(seed),
...
def main():
    ...
    try:
        ...
        if args.smoke_only:
            (shard_root / "status").write_text("SMOKE_OK\n")
            return
