...
: "${ACTOR_ROWS:?ACTOR_ROWS must be 64 or 256}"
if [[ "$ACTOR_ROWS" != "64" && "$ACTOR_ROWS" != "256" ]]; then
  echo "invalid ACTOR_ROWS=$ACTOR_ROWS" >&2
  exit 2
fi
...
RUN_ROOT="$ROOT/perf_runs/csf3_detjc_emp${ACTOR_ROWS}_fullrhs_adamcritic_kfalse_normnone_damp01_all7_5seed_10m_gpuL3_${SLURM_ARRAY_JOB_ID}"
...
family=detjc_emp${ACTOR_ROWS}_fullrhs_adamcritic_kfalse_normnone_damp01
mode=no_shared_emp${ACTOR_ROWS}_full_minibatch_rhs_standard_adam_critic
trainer=train_detach_jointcritic.py
trainer_sha256=$(sha256sum train_detach_jointcritic.py | awk '{print $1}')
worker=run_noshared_emp_fullrhs_adamcritic_kfalse_worker.py
worker_sha256=$(sha256sum run_noshared_emp_fullrhs_adamcritic_kfalse_worker.py | awk '{print $1}')
...
launcher=sbatch_noshared_emp_fullrhs_adamcritic_kfalse_gpuL3.sh
launcher_sha256=$(sha256sum sbatch_noshared_emp_fullrhs_adamcritic_kfalse_gpuL3.sh | awk '{print $1}')
config=rat_mlp_detjc_emp_fullrhs_adamcritic_kfalse.yaml
config_sha256=$(sha256sum configs/rat_mlp_detjc_emp_fullrhs_adamcritic_kfalse.yaml | awk '{print $1}')
...
actor_curvature_subsample=$ACTOR_ROWS
...
"$PY" -u "$ROOT/run_noshared_emp_fullrhs_adamcritic_kfalse_worker.py" \
  --run-root "$RUN_ROOT" \
  --repo "$ROOT" \
  --python "$PY" \
