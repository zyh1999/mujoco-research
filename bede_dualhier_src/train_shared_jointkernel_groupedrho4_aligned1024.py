...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def RAT_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        # Energy-free actor basis. The default keeps 255 anchor coefficients
        # plus one non-anchor rho. actor_group_size instead partitions the full
        # minibatch into equally sized groups with alpha_i=rho_g*b_i.
        ...
        if use_grouped_rho:
            if _obs.shape[0] % actor_group_size != 0:
                raise ValueError('minibatch size must be divisible by actor_group_size')
            if actor_curvature_subsample != _obs.shape[0] // actor_group_size:
                raise ValueError('actor_curvature_subsample must equal the number of grouped-rho degrees of freedom')
            if critic_curvature_subsample != 256:
                raise ValueError('grouped-rho shared trainer requires critic_sub=256')
        elif actor_curvature_subsample != 255 or critic_curvature_subsample != 256:
            raise ValueError('energy-free shared joint trainer requires actor_sub=255 and critic_sub=256')
        if _obs.shape[0] <= critic_curvature_subsample:
        ...
        shared_perm = torch.randperm(_obs.shape[0], device=_obs.device)
        critic_curvature_inds = shared_perm[:critic_curvature_subsample]
        if use_grouped_rho:
            actor_curvature_inds = None
            nonactor_inds = None
        else:
            actor_curvature_inds = critic_curvature_inds[:actor_curvature_subsample]
            actor_mask = torch.zeros(_obs.shape[0], device=_obs.device, dtype=torch.bool)
            actor_mask[actor_curvature_inds] = True
            nonactor_inds = torch.arange(_obs.shape[0], device=_obs.device)[~actor_mask]

        _critic_obs = _obs[critic_curvature_inds]
        _critic_ret = _ret[critic_curvature_inds]
        _critic_vals = _vals[critic_curvature_inds]
        ...
        ft_v_grads = ft_compute_v_grad(dict_params, dict_buffers, _critic_obs)

        with torch.no_grad():
            full_actor_sa = _obs.shape[0]
            num_actor_anchors = 0 if use_grouped_rho else actor_curvature_subsample
            num_actor_sa = actor_curvature_subsample if use_grouped_rho else num_actor_anchors + 1
            num_critic_sa = _critic_obs.shape[0]
            kernel_denom = float(num_actor_sa)
            H_pi_full = torch.cat(
                [ft_pi_grads_full[name].view(full_actor_sa, -1) for name in param_names],
                dim=-1,
            )
            if not use_grouped_rho:
                H_pi_s = H_pi_full.index_select(0, actor_curvature_inds)
                H_pi_n = H_pi_full.index_select(0, nonactor_inds)
            J_v = torch.cat([ft_v_grads[name].view(num_critic_sa, -1) for name in param_names], dim=-1)
            ...
            if g_k is None:
                ...
                prev_critic = torch.zeros_like(_critic_ret)
            else:
                prev_actor_full = H_pi_full @ g_k
                prev_critic = J_v @ g_k

            if use_grouped_rho:
                residual_full = target_full - prev_actor_full
                grouped_H = H_pi_full.index_select(0, shared_perm).view(num_actor_sa, actor_group_size, -1)
                grouped_b = residual_full.index_select(0, shared_perm).view(num_actor_sa, actor_group_size)
                grouped_ratio = ratio_full.index_select(0, shared_perm).view(num_actor_sa, actor_group_size)
                actor_basis = torch.einsum(
                    'grp,gr->gp', grouped_H, grouped_ratio * grouped_b
                )
                actor_test = actor_basis
                actor_trial = actor_basis
                actor_metric = (
                    grouped_ratio * grouped_b.square()
                ).sum(dim=1).clamp_min(1e-12)
            else:
                target_s = target_full.index_select(0, actor_curvature_inds)
                target_n = target_full.index_select(0, nonactor_inds)
                ratio_s = ratio_full.index_select(0, actor_curvature_inds)
                ratio_n = ratio_full.index_select(0, nonactor_inds)
                rhs_s = target_s - prev_actor_full.index_select(0, actor_curvature_inds)
                b_n = target_n - prev_actor_full.index_select(0, nonactor_inds)
                rho_metric = torch.dot(b_n, b_n).clamp_min(1e-12)
            ...
            critic_basis = vf_weight * J_v
            joint_test = torch.cat([actor_test, critic_basis], dim=0)
            joint_trial = torch.cat([actor_trial, critic_basis], dim=0)
            critic_rhs = vf_weight * (
                (_critic_ret - _critic_vals).detach() - prev_critic
            )
            if use_grouped_rho:
                joint_rhs = torch.cat([actor_metric, critic_rhs], dim=0)
                damping_metric = torch.cat([actor_metric, torch.ones_like(critic_rhs)], dim=0)
                # Preserve the original normalizations of both blocks. The actor
                # basis represents all 1024 samples, while the critic still uses
                # its unchanged 256 sampled GGN rows.
                joint_trial_scaled = torch.cat(
                    [actor_trial / float(full_actor_sa), critic_basis / float(num_critic_sa)],
                    dim=0,
                )
                reduced_system = joint_test @ joint_trial_scaled.t()
            ...
            if use_grouped_rho:
                ...
                grouped_rho_std = grouped_rho_min = grouped_rho_max = 0.0
        ...
        with torch.no_grad():
            ...
            pi_info = dict(kl=_real_kl.item(), curr_kl=_curr_kl.item(), curr_lr=ac_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_group_kernel_denominator=(full_actor_sa if use_grouped_rho else num_actor_sa),
                           actor_group_damping_ratio_weighted=int(use_grouped_rho),
                           critic_curvature_samples=num_critic_sa,
                           shared_curvature_indices=1)
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
                        'halfcheetah', 'walker2d', 'humanoid', 'humanoidstandup', 'reacher', 'swimmer', 'ant'}:
            log_dir = f"logs/shared_groupedrho4_aligned1024.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}.a{nets_config.hidden_size}x{nets_config.num_layers}x{nets_config.dropout}e{algo_config.epochs}x{algo_config.minibatches}.clip_grad_{algo_config.max_grad_norm}.{algo_config.sigma_type}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}{subcurve_tag}/{env_name}.{time_now}_{seed}"
        else:
            log_dir = f"logs/shared_groupedrho4_aligned1024.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}{'_bn' if nets_config.with_bn else ''}.dropout_{nets_config.dropout}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}{subcurve_tag}/{env_config.env_name}.{time_now}_{seed}"
