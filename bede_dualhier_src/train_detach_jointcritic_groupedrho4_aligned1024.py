    ...
    def RAT_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        if use_grouped_rho:
            ...
            with torch.no_grad():
                ...
                ).sum(dim=1).clamp_min(metric_eps)
                reduced_system = actor_basis @ actor_basis.t() / float(full_m)
                reduced_system = reduced_system + damping * torch.diag(group_metric)
                grouped_rho = torch.linalg.solve(reduced_system, group_metric)
                update_dir = actor_basis.t().matmul(grouped_rho) / float(full_m)
        ...
        with torch.no_grad():
            ...
            pi_info = dict(kl=_real_kl.item(), curr_lr=pi_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_group_kernel_denominator=(_obs.shape[0] if use_grouped_rho else num_sa),
