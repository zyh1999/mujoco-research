...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def RAT_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        # Actor and critic each contribute 16 local 64-sample directions. Their
        # shared parameters couple all 32 directions in the top-level solve.
        group_size = int(getattr(algo_config, 'dual_hierarchical_group_size', 0) or 0)
        if group_size <= 0 or _obs.shape[0] % group_size != 0:
            raise ValueError('dual_hierarchical_group_size must evenly divide the minibatch')
        if (getattr(algo_config, 'actor_curvature_subsample', 0) or 0) != 0:
            raise ValueError('shared dual hierarchy requires full actor curvature rows')
        if (getattr(algo_config, 'critic_curvature_subsample', 0) or 0) != 0:
            raise ValueError('shared dual hierarchy requires full critic curvature rows')
        if algo_config.is_karzmarz:
            raise ValueError('shared dual hierarchy currently requires Kaczmarz=false')
        if str(algo_config.fisher_kernel_normalization).lower() != 'none':
            raise ValueError('shared dual hierarchy currently requires normalization=none')
        if str(algo_config.fisher_kernel).lower() != 'exact':
            raise NotImplementedError('shared dual hierarchy currently requires fisher_kernel=exact')
        damping = float(algo_config.cg_damping)
        if damping <= 0.0:
            raise ValueError('shared dual hierarchy requires positive cg_damping')
        ...
        with torch.no_grad():
            ...
            num_groups = full_actor_sa // group_size
            H_pi_full = torch.cat(
            ...
            vf_weight = math.sqrt(float(algo_config.vf_coef))
            target_full = _adv.detach().to(dtype=H_pi_full.dtype)
            ratio_full = _ratio.detach().to(dtype=H_pi_full.dtype)
            critic_target_full = vf_weight * (_ret - _vals).detach().to(dtype=J_v_full.dtype)
            critic_features = vf_weight * J_v_full
            metric_eps = float(getattr(algo_config, 'fisher_kernel_normalization_eps', 1e-12))

            actor_perm = torch.randperm(full_actor_sa, device=H_pi_full.device)
            grouped_H = H_pi_full.index_select(0, actor_perm).view(num_groups, group_size, -1)
            grouped_actor_b = target_full.index_select(0, actor_perm).view(num_groups, group_size)
            grouped_ratio = ratio_full.index_select(0, actor_perm).view(num_groups, group_size)
            actor_local_kernel = torch.bmm(grouped_H, grouped_H.transpose(1, 2)) / float(full_actor_sa)
            local_eye = torch.eye(group_size, device=H_pi_full.device, dtype=H_pi_full.dtype).expand(
                num_groups, -1, -1
            )
            actor_local_system = actor_local_kernel * grouped_ratio.unsqueeze(1) + damping * local_eye
            actor_local_alpha = torch.linalg.solve(
                actor_local_system, grouped_actor_b.unsqueeze(-1)
            ).squeeze(-1)
            actor_basis = torch.bmm(
                grouped_H.transpose(1, 2),
                (grouped_ratio * actor_local_alpha).unsqueeze(-1),
            ).squeeze(-1)
            actor_metric = (grouped_ratio * actor_local_alpha.square()).sum(dim=1).clamp_min(metric_eps)
            actor_rhs = (grouped_ratio * actor_local_alpha * grouped_actor_b).sum(dim=1)
            actor_local_residual = torch.bmm(
                actor_local_system, actor_local_alpha.unsqueeze(-1)
            ).squeeze(-1) - grouped_actor_b
            actor_local_relative = actor_local_residual.norm(dim=1) / grouped_actor_b.norm(dim=1).clamp_min(1e-12)

            critic_perm = torch.randperm(full_actor_sa, device=J_v_full.device)
            grouped_C = critic_features.index_select(0, critic_perm).view(num_groups, group_size, -1)
            grouped_critic_b = critic_target_full.index_select(0, critic_perm).view(num_groups, group_size)
            critic_local_kernel = torch.bmm(grouped_C, grouped_C.transpose(1, 2)) / float(full_actor_sa)
            critic_local_system = critic_local_kernel + damping * local_eye
            critic_local_alpha = torch.linalg.solve(
                critic_local_system, grouped_critic_b.unsqueeze(-1)
            ).squeeze(-1)
            critic_basis = torch.bmm(
                grouped_C.transpose(1, 2), critic_local_alpha.unsqueeze(-1)
            ).squeeze(-1)
            critic_metric = critic_local_alpha.square().sum(dim=1).clamp_min(metric_eps)
            critic_rhs = (critic_local_alpha * grouped_critic_b).sum(dim=1)
            critic_local_residual = torch.bmm(
                critic_local_system, critic_local_alpha.unsqueeze(-1)
            ).squeeze(-1) - grouped_critic_b
            critic_local_relative = critic_local_residual.norm(dim=1) / grouped_critic_b.norm(dim=1).clamp_min(1e-12)

            joint_basis = torch.cat([actor_basis, critic_basis], dim=0)
            damping_metric = torch.cat([actor_metric, critic_metric], dim=0)
            joint_rhs = torch.cat([actor_rhs, critic_rhs], dim=0)
            reduced_system = joint_basis @ joint_basis.t() / float(full_actor_sa)
            reduced_system = reduced_system + damping * torch.diag(damping_metric)
            global_scale = reduced_system.diagonal().abs().mean().clamp_min(1.0)
            reduced_system = reduced_system + metric_eps * global_scale * torch.eye(
                2 * num_groups, device=H_pi_full.device, dtype=H_pi_full.dtype
            )
            joint_rho = torch.linalg.solve(reduced_system, joint_rhs)
            flat_dir = joint_basis.t().matmul(joint_rho) / float(full_actor_sa)
            global_residual = reduced_system @ joint_rho - joint_rhs

            actor_local_relative_residual = float(actor_local_relative.mean().item())
            critic_local_relative_residual = float(critic_local_relative.mean().item())
            global_relative_residual = float(
                (global_residual.norm() / joint_rhs.norm().clamp_min(1e-12)).item()
            )
            actor_rho = joint_rho[:num_groups]
            critic_rho = joint_rho[num_groups:]
        ...
        with torch.no_grad():
            ...
            pi_info = dict(kl=_real_kl.item(), curr_kl=_curr_kl.item(), curr_lr=ac_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_curvature_samples=full_actor_sa, actor_anchor_samples=0,
                           actor_free_rho_value=float(actor_rho.mean().item()),
                           actor_grouped_rho=0,
                           critic_curvature_samples=full_actor_sa,
                           shared_curvature_indices=0,
                           dual_hierarchical=1,
                           dual_hierarchical_group_size=group_size,
                           dual_hierarchical_num_groups=num_groups,
                           dual_hierarchical_top_size=2 * num_groups,
                           actor_hierarchical_local_relative_residual=actor_local_relative_residual,
                           critic_hierarchical_local_relative_residual=critic_local_relative_residual,
                           dual_hierarchical_global_relative_residual=global_relative_residual,
                           actor_hierarchical_rho_std=float(actor_rho.std(unbiased=False).item()),
                           critic_hierarchical_rho_mean=float(critic_rho.mean().item()),
                           critic_hierarchical_rho_std=float(critic_rho.std(unbiased=False).item()))
    ...
    for epoch in tepochs:
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if pi_info.get('dual_hierarchical', 0):
                logger.logkv("dual_hierarchical", 1)
                logger.logkv("dual_hierarchical_group_size", pi_info['dual_hierarchical_group_size'])
                logger.logkv("dual_hierarchical_num_groups", pi_info['dual_hierarchical_num_groups'])
                logger.logkv("dual_hierarchical_top_size", pi_info['dual_hierarchical_top_size'])
                logger.logkv(
                ...
                logger.logkv(
                    ...
                    pi_info['critic_hierarchical_local_relative_residual'],
                )
                logger.logkv(
                    "dual_hierarchical_global_relative_residual",
                    pi_info['dual_hierarchical_global_relative_residual'],
                )
                logger.logkv("actor_hierarchical_rho_std", pi_info['actor_hierarchical_rho_std'])
                logger.logkv("critic_hierarchical_rho_mean", pi_info['critic_hierarchical_rho_mean'])
                logger.logkv("critic_hierarchical_rho_std", pi_info['critic_hierarchical_rho_std'])
            if pi_info.get('actor_grouped_rho', 0):
                ...
                logger.logkv("actor_group_kernel_denominator", pi_info['actor_group_kernel_denominator'])
                logger.logkv("actor_group_damping_ratio_weighted", pi_info['actor_group_damping_ratio_weighted'])
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
                        'halfcheetah', 'walker2d', 'humanoid', 'humanoidstandup', 'reacher', 'swimmer', 'ant'}:
            log_dir = f"logs/shared_dualhier16x64_top32_aligned1024.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}.a{nets_config.hidden_size}x{nets_config.num_layers}x{nets_config.dropout}e{algo_config.epochs}x{algo_config.minibatches}.clip_grad_{algo_config.max_grad_norm}.{algo_config.sigma_type}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}{subcurve_tag}/{env_name}.{time_now}_{seed}"
        else:
            log_dir = f"logs/shared_dualhier16x64_top32_aligned1024.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}{'_bn' if nets_config.with_bn else ''}.dropout_{nets_config.dropout}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}{subcurve_tag}/{env_config.env_name}.{time_now}_{seed}"
...
def main():
    ...
    parser.add_argument('--actor_group_size', type=int, default=None,
                        help='Tie alpha_i=rho_g*b_i within fixed-size random actor groups')
    parser.add_argument('--dual_hierarchical_group_size', type=int, default=None,
                        help='Local actor/critic group size before the joint top-level solve')
    ...
    if not hasattr(algo_config, 'dual_hierarchical_group_size'):
        algo_config.dual_hierarchical_group_size = 0
    ...
    if args.dual_hierarchical_group_size is not None:
        algo_config.dual_hierarchical_group_size = args.dual_hierarchical_group_size
