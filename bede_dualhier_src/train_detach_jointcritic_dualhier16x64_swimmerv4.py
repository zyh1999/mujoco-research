...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if critic_optimizer_name == 'adam':
        ...
        v_optimizer = SGD(params_v, lr=algo_config.lr_v, momentum=1e-6)
    else:
    ...
    def RAT_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        # The hierarchical path first solves independent coefficient systems,
        # then restricts the original full-minibatch energy to their span.
        actor_group_size = int(getattr(algo_config, 'actor_group_size', 0) or 0)
        use_grouped_rho = actor_group_size > 0
        actor_hierarchical_group_size = int(
            getattr(algo_config, 'actor_hierarchical_group_size', 0) or 0
        )
        use_hierarchical = actor_hierarchical_group_size > 0
        if use_grouped_rho and use_hierarchical:
            raise ValueError("actor_group_size and actor_hierarchical_group_size are mutually exclusive")
        actor_curvature_subsample = getattr(algo_config, 'actor_curvature_subsample', 0) or 0
        if use_grouped_rho or use_hierarchical:
        ...
        grouped_rho_min = 0.0
        grouped_rho_max = 0.0
        hierarchical_local_relative_residual = 0.0
        hierarchical_global_relative_residual = 0.0
        hierarchical_local_direction_norm = 0.0
        hierarchical_rho_mean = 0.0
        hierarchical_rho_std = 0.0
        hierarchical_rho_min = 0.0
        hierarchical_rho_max = 0.0
        ...
        with torch.no_grad():
            ...
            if use_actor_identity_kernel:
                ...
                _png_adv = None
            elif use_free_rho or use_hierarchical:
                _png_adv = None
            else:
        ...
        if use_hierarchical:
            ...
            with torch.no_grad():
                ...
                if damping <= 0.0:
                    raise ValueError("actor_hierarchical_group_size requires positive cg_damping")
                if algo_config.is_karzmarz:
                    raise ValueError("hierarchical actor currently requires Kaczmarz=false")
                if str(getattr(algo_config, 'fisher_kernel', 'exact')).lower() != 'exact':
                    raise NotImplementedError("hierarchical actor currently requires fisher_kernel=exact")
                if str(getattr(algo_config, 'fisher_kernel_normalization', 'none')).lower() != 'none':
                    raise ValueError("hierarchical actor currently requires fisher_kernel_normalization=none")

                full_m = int(H.shape[0])
                group_size = actor_hierarchical_group_size
                if full_m % group_size != 0:
                    raise ValueError(
                        f"minibatch size {full_m} is not divisible by "
                        f"actor_hierarchical_group_size={group_size}"
                    )
                num_groups = full_m // group_size
                group_perm = torch.randperm(full_m, device=H.device)
                grouped_H = H.index_select(0, group_perm).view(num_groups, group_size, -1)
                target_full = _adv.detach().to(device=H.device, dtype=H.dtype)
                ratio_full = _ratio.detach().to(device=H.device, dtype=H.dtype)
                grouped_b = target_full.index_select(0, group_perm).view(num_groups, group_size)
                grouped_ratio = ratio_full.index_select(0, group_perm).view(num_groups, group_size)

                # Level 1: solve each diagonal block of the original
                # M-sample coefficient system. Dividing by full_m keeps both
                # the kernel and damping on the same scale as the full solve.
                local_kernel = torch.bmm(grouped_H, grouped_H.transpose(1, 2)) / float(full_m)
                local_eye = torch.eye(group_size, device=H.device, dtype=H.dtype).expand(
                    num_groups, -1, -1
                )
                local_system = local_kernel * grouped_ratio.unsqueeze(1) + damping * local_eye
                local_alpha = torch.linalg.solve(local_system, grouped_b.unsqueeze(-1)).squeeze(-1)
                local_weighted_alpha = grouped_ratio * local_alpha
                actor_basis = torch.bmm(
                    grouped_H.transpose(1, 2), local_weighted_alpha.unsqueeze(-1)
                ).squeeze(-1)

                local_residual = torch.bmm(
                    local_system, local_alpha.unsqueeze(-1)
                ).squeeze(-1) - grouped_b
                local_relative = local_residual.norm(dim=1) / grouped_b.norm(dim=1).clamp_min(1e-12)

                # Level 2: alpha_i = rho_g * local_alpha_i. Restricting the
                # original coefficient energy gives
                #   (B B^T / M + mu diag(q)) rho = c,
                # where B_g=sum_i r_i*a_i*h_i, q_g=sum_i r_i*a_i^2,
                # and c_g=sum_i r_i*a_i*b_i.
                metric_eps = float(getattr(algo_config, 'actor_subsample_free_rho_eps', 1e-12))
                group_metric = (grouped_ratio * local_alpha.square()).sum(dim=1).clamp_min(
                    metric_eps
                )
                group_rhs = (grouped_ratio * local_alpha * grouped_b).sum(dim=1)
                global_system = actor_basis @ actor_basis.t() / float(full_m)
                global_system = global_system + damping * torch.diag(group_metric)
                global_rhs = group_rhs

                global_scale = global_system.diagonal().abs().mean().clamp_min(1.0)
                global_system = global_system + metric_eps * global_scale * torch.eye(
                    num_groups, device=H.device, dtype=H.dtype
                )
                hierarchical_rho = torch.linalg.solve(global_system, global_rhs)
                update_dir = actor_basis.t().matmul(hierarchical_rho) / float(full_m)
                global_residual = global_system @ hierarchical_rho - global_rhs

                set_grads_from_flat(params_pi, -update_dir)
                hierarchical_local_relative_residual = float(local_relative.mean().item())
                hierarchical_global_relative_residual = float(
                    (global_residual.norm() / global_rhs.norm().clamp_min(1e-12)).item()
                )
                hierarchical_local_direction_norm = float(
                    (actor_basis / float(full_m)).norm(dim=1).mean().item()
                )
                hierarchical_rho_mean = float(hierarchical_rho.mean().item())
                hierarchical_rho_std = float(hierarchical_rho.std(unbiased=False).item())
                hierarchical_rho_min = float(hierarchical_rho.min().item())
                hierarchical_rho_max = float(hierarchical_rho.max().item())
        ...
        with torch.no_grad():
            ...
            pi_info = dict(kl=_real_kl.item(), curr_lr=pi_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_hierarchical=int(use_hierarchical),
                           actor_hierarchical_group_size=actor_hierarchical_group_size,
                           actor_hierarchical_num_groups=(
                               int(num_sa // actor_hierarchical_group_size) if use_hierarchical else 0
                           ),
                           actor_hierarchical_local_relative_residual=hierarchical_local_relative_residual,
                           actor_hierarchical_global_relative_residual=hierarchical_global_relative_residual,
                           actor_hierarchical_local_direction_norm=hierarchical_local_direction_norm,
                           actor_hierarchical_rho_mean=hierarchical_rho_mean,
                           actor_hierarchical_rho_std=hierarchical_rho_std,
                           actor_hierarchical_rho_min=hierarchical_rho_min,
                           actor_hierarchical_rho_max=hierarchical_rho_max)
    ...
    def CriticGN_Update(_obs, _ret):
        ...
        critic_hierarchical_group_size = int(
            getattr(algo_config, 'critic_hierarchical_group_size', 0) or 0
        )
        use_critic_hierarchical = critic_hierarchical_group_size > 0
        critic_curvature_subsample = getattr(algo_config, 'critic_curvature_subsample', 0) or 0
        if use_critic_hierarchical:
            _critic_obs = _obs
            _critic_residual = _residual
        elif critic_curvature_subsample > 0 and _obs.shape[0] > critic_curvature_subsample:
        ...
        with torch.no_grad():
            ...
            if use_critic_hierarchical:
                damping = float(algo_config.cg_damping)
                if damping <= 0.0:
                    raise ValueError("critic_hierarchical_group_size requires positive cg_damping")
                if str(getattr(algo_config, 'fisher_kernel', 'exact')).lower() != 'exact':
                    raise NotImplementedError("hierarchical critic currently requires fisher_kernel=exact")
                if str(getattr(algo_config, 'fisher_kernel_normalization', 'none')).lower() != 'none':
                    raise ValueError("hierarchical critic currently requires fisher_kernel_normalization=none")
                if num_samples % critic_hierarchical_group_size != 0:
                    raise ValueError(
                        f"critic minibatch size {num_samples} is not divisible by "
                        f"critic_hierarchical_group_size={critic_hierarchical_group_size}"
                    )

                num_groups = num_samples // critic_hierarchical_group_size
                group_perm = torch.randperm(num_samples, device=J.device)
                grouped_J = J.index_select(0, group_perm).view(
                    num_groups, critic_hierarchical_group_size, -1
                )
                grouped_b = _critic_residual.to(device=J.device, dtype=J.dtype).index_select(
                    0, group_perm
                ).view(num_groups, critic_hierarchical_group_size)

                local_kernel = torch.bmm(grouped_J, grouped_J.transpose(1, 2)) / float(num_samples)
                local_eye = torch.eye(
                    critic_hierarchical_group_size, device=J.device, dtype=J.dtype
                ).expand(num_groups, -1, -1)
                local_system = local_kernel + damping * local_eye
                local_alpha = torch.linalg.solve(local_system, grouped_b.unsqueeze(-1)).squeeze(-1)
                critic_basis = torch.bmm(
                    grouped_J.transpose(1, 2), local_alpha.unsqueeze(-1)
                ).squeeze(-1)
                ...
                local_relative = local_residual.norm(dim=1) / grouped_b.norm(dim=1).clamp_min(1e-12)
                metric_eps = float(getattr(algo_config, 'actor_subsample_free_rho_eps', 1e-12))
                group_metric = local_alpha.square().sum(dim=1).clamp_min(metric_eps)
                group_rhs = (local_alpha * grouped_b).sum(dim=1)
                global_system = critic_basis @ critic_basis.t() / float(num_samples)
                global_system = global_system + damping * torch.diag(group_metric)
                global_scale = global_system.diagonal().abs().mean().clamp_min(1.0)
                global_system = global_system + metric_eps * global_scale * torch.eye(
                    num_groups, device=J.device, dtype=J.dtype
                )
                hierarchical_rho = torch.linalg.solve(global_system, group_rhs)
                step_dir = critic_basis.t().matmul(hierarchical_rho) / float(num_samples)
                global_residual = global_system @ hierarchical_rho - group_rhs
                critic_local_relative_residual = float(local_relative.mean().item())
                critic_global_relative_residual = float(
                    (global_residual.norm() / group_rhs.norm().clamp_min(1e-12)).item()
                )
                critic_rho_mean = float(hierarchical_rho.mean().item())
                critic_rho_std = float(hierarchical_rho.std(unbiased=False).item())
            else:
                ...
                critic_local_relative_residual = 0.0
                critic_global_relative_residual = 0.0
                critic_rho_mean = 0.0
                critic_rho_std = 0.0
        ...
        with torch.no_grad():
            v_info = dict(
                ...
                v_step_norm=step_dir.norm().item(),
                critic_curvature_samples=num_samples,
                critic_hierarchical=int(use_critic_hierarchical),
                critic_hierarchical_group_size=critic_hierarchical_group_size,
                critic_hierarchical_num_groups=(
                    num_samples // critic_hierarchical_group_size
                    if use_critic_hierarchical else 0
                ),
                critic_hierarchical_local_relative_residual=critic_local_relative_residual,
                critic_hierarchical_global_relative_residual=critic_global_relative_residual,
                critic_hierarchical_rho_mean=critic_rho_mean,
                critic_hierarchical_rho_std=critic_rho_std,
            )
    ...
    def CriticStandard_Update(_obs, _ret):
        ...
        with torch.no_grad():
            v_info = dict(v_grad_norm=grad_norm.item(), v_step_norm=0.0,
                          critic_curvature_samples=0, critic_hierarchical=0)
        return mb_loss_v, v_info
    ...
    for epoch in tepochs:
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if v_info.get('critic_hierarchical', 0):
                logger.logkv("critic_hierarchical", 1)
                logger.logkv("critic_hierarchical_group_size", v_info['critic_hierarchical_group_size'])
                logger.logkv("critic_hierarchical_num_groups", v_info['critic_hierarchical_num_groups'])
                logger.logkv(
                    "critic_hierarchical_local_relative_residual",
                    v_info['critic_hierarchical_local_relative_residual'],
                )
                logger.logkv(
                    "critic_hierarchical_global_relative_residual",
                    v_info['critic_hierarchical_global_relative_residual'],
                )
                logger.logkv("critic_hierarchical_rho_mean", v_info['critic_hierarchical_rho_mean'])
                logger.logkv("critic_hierarchical_rho_std", v_info['critic_hierarchical_rho_std'])
            ...
            if pi_info.get('actor_hierarchical', 0):
                logger.logkv("actor_hierarchical", 1)
                logger.logkv("actor_hierarchical_group_size", pi_info['actor_hierarchical_group_size'])
                logger.logkv("actor_hierarchical_num_groups", pi_info['actor_hierarchical_num_groups'])
                logger.logkv(
                    "actor_hierarchical_local_relative_residual",
                    pi_info['actor_hierarchical_local_relative_residual'],
                )
                logger.logkv(
                    "actor_hierarchical_global_relative_residual",
                    pi_info['actor_hierarchical_global_relative_residual'],
                )
                logger.logkv(
                    "actor_hierarchical_local_direction_norm",
                    pi_info['actor_hierarchical_local_direction_norm'],
                )
                logger.logkv("actor_hierarchical_rho_mean", pi_info['actor_hierarchical_rho_mean'])
                logger.logkv("actor_hierarchical_rho_std", pi_info['actor_hierarchical_rho_std'])
                logger.logkv("actor_hierarchical_rho_min", pi_info['actor_hierarchical_rho_min'])
                logger.logkv("actor_hierarchical_rho_max", pi_info['actor_hierarchical_rho_max'])
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        if hierarchical_group_size:
            actor_minibatch_size = (
                env_config.num_envs * env_config.nsteps // algo_config.pi_minibatches
            )
            hierarchy_tag = (
                f".hier{actor_minibatch_size // hierarchical_group_size}x"
                f"{hierarchical_group_size}.critic"
                f"{actor_minibatch_size // critic_hierarchical_group_size}x"
                f"{critic_hierarchical_group_size}_aligned1024"
            )
        else:
            hierarchy_tag = ""
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'carracing', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
                        'halfcheetah', 'walker2d', 'humanoid', 'humanoidstandup', 'reacher', 'swimmer', 'ant'}:
            log_dir = f"logs/{algo}{hierarchy_tag}.jointcritic.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}.a{nets_config.a_hidden_size}x{nets_config.a_num_layers}x{nets_config.a_dropout}e{algo_config.pi_epochs}x{algo_config.pi_minibatches}.c{nets_config.c_hidden_size}x{nets_config.c_num_layers}x{nets_config.c_dropout}e{algo_config.v_epochs}x{algo_config.v_minibatches}.{algo_config.grad}_{algo_config.post_grad}_{algo_config.max_grad_norm}.{algo_config.sigma_type}.damping_{algo_config.cg_damping}.lr_pi_{algo_config.lr_pi}.lr_v_{algo_config.lr_v}{subcurve_tag}/{env_name}.{time_now}_{seed}"
        else:
...
def main():
    ...
    parser.add_argument('--actor_hierarchical_group_size', type=int, default=None,
                        help='Solve local within-group actor systems, then a global energy solve over one direction per group')
    parser.add_argument('--critic_hierarchical_group_size', type=int, default=None,
                        help='Solve local within-group critic systems, then a global energy solve over one direction per group')
    ...
    if not hasattr(algo_config, 'actor_hierarchical_group_size'):
        algo_config.actor_hierarchical_group_size = 0
    if not hasattr(algo_config, 'critic_hierarchical_group_size'):
        algo_config.critic_hierarchical_group_size = 0
    ...
    if args.actor_hierarchical_group_size is not None:
        algo_config.actor_hierarchical_group_size = args.actor_hierarchical_group_size
    if args.critic_hierarchical_group_size is not None:
        algo_config.critic_hierarchical_group_size = args.critic_hierarchical_group_size
