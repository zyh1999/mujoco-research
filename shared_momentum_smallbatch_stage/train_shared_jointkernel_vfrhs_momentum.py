...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def RAT_Update(_obs, _act, _adv, _ret, _outputs_old):
        ...
        # Per-sample policy scores and value Jacobians form a block system:
        # [H_pi H_pi^T, H_pi_shared J_v_shared^T; J_v_shared H_pi_shared^T, J_v J_v^T].
        # When both sides request the same subsample size, reuse the same rows.
        ...
        shared_curvature_inds = None
        if (
            actor_curvature_subsample > 0
            and actor_curvature_subsample == critic_curvature_subsample
            and _obs.shape[0] > actor_curvature_subsample
        ):
            shared_curvature_inds = torch.randperm(_obs.shape[0], device=_obs.device)[:actor_curvature_subsample]

        if shared_curvature_inds is not None:
            actor_curvature_inds = shared_curvature_inds
        ...
        if shared_curvature_inds is not None:
            critic_curvature_inds = shared_curvature_inds
        ...
        ft_pi_grads = ft_compute_pi_grad(dict_params, dict_buffers, _actor_obs, _actor_act)
        ft_v_grads = ft_compute_v_grad(dict_params, dict_buffers, _critic_obs)

        with torch.no_grad():
            num_actor_sa = _actor_obs.shape[0]
            num_critic_sa = _critic_obs.shape[0]
            kernel_denom = float(num_actor_sa) if num_actor_sa == num_critic_sa else math.sqrt(float(num_actor_sa * num_critic_sa))
            H_pi = torch.cat([ft_pi_grads[name].view(num_actor_sa, -1) for name in param_names], dim=-1)
            J_v = torch.cat([ft_v_grads[name].view(num_critic_sa, -1) for name in param_names], dim=-1)

            # Keep the joint curvature metric fixed and use vf_coef only as
            # the relative value-gradient target weight.
            joint_H = torch.cat([H_pi, J_v], dim=0)
            joint_rhs = torch.cat([
                _actor_adv,
                float(algo_config.vf_coef) * (_critic_ret - _critic_vals).detach(),
            ], dim=0)
            joint_ratio = torch.cat([_actor_ratio.detach(), torch.ones_like(_critic_ret)], dim=0)
            ...
            K = joint_H @ joint_H.t() / kernel_denom
            K, row_scale = normalize_score_kernel(
                K,
                normalization=algo_config.fisher_kernel_normalization,
                eps=algo_config.fisher_kernel_normalization_eps,
            )
            ...
            if algo_config.is_karzmarz:
                ...
                if has_momentum:
                    previous_projection = joint_H @ torch.cat(gk_list, dim=0)
                    if row_scale is not None:
                        previous_projection = row_scale.to(dtype=previous_projection.dtype) * previous_projection

            rhs_eff = joint_rhs.to(dtype=K.dtype)
            if row_scale is not None:
                rhs_eff = row_scale.to(dtype=rhs_eff.dtype) * rhs_eff
            if previous_projection is not None:
                rhs_eff = rhs_eff - previous_projection.to(dtype=rhs_eff.dtype)

            K = K * joint_ratio.to(dtype=K.dtype).unsqueeze(0)
            eye = torch.eye(K.shape[0], device=K.device, dtype=K.dtype)
            joint_alpha = torch.linalg.solve(K + algo_config.cg_damping * eye, rhs_eff)
            if row_scale is not None:
                joint_alpha = row_scale.to(dtype=joint_alpha.dtype) * joint_alpha

            weighted_alpha = joint_ratio.to(dtype=joint_alpha.dtype) * joint_alpha
            flat_dir = joint_H.t().matmul(weighted_alpha) / kernel_denom
        ...
        with torch.no_grad():
            ...
            pi_info = dict(kl=_real_kl.item(), curr_kl=_curr_kl.item(), curr_lr=ac_optimizer.param_groups[0]['lr'], ent=_entropy.item(), cf=clipfrac, 
                           ...
                           actor_curvature_samples=num_actor_sa, critic_curvature_samples=num_critic_sa,
                           shared_curvature_indices=int(shared_curvature_inds is not None))
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
                        'halfcheetah', 'walker2d', 'humanoid', 'humanoidstandup', 'reacher', 'swimmer', 'ant'}:
            log_dir = f"logs/shared_vfrhs.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}.a{nets_config.hidden_size}x{nets_config.num_layers}x{nets_config.dropout}e{algo_config.epochs}x{algo_config.minibatches}.clip_grad_{algo_config.max_grad_norm}.{algo_config.sigma_type}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}.vf_{algo_config.vf_coef}{subcurve_tag}/{env_name}.{time_now}_{seed}"
        else:
            log_dir = f"logs/shared_vfrhs.{algo}.karzmarz_{algo_config.is_karzmarz}.{nets_config.type}{'_bn' if nets_config.with_bn else ''}.dropout_{nets_config.dropout}.damping_{algo_config.cg_damping}.lr_{algo_config.lr}.vf_{algo_config.vf_coef}{subcurve_tag}/{env_config.env_name}.{time_now}_{seed}"

        format_strs = ['csv', 'stdout'] 
        logger.configure(dir=log_dir, format_strs=format_strs)
        logger.info(f"joint_vf_mode=rhs_only vf_coef={algo_config.vf_coef}")
