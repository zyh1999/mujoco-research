...
cat > "$RUN_ROOT/run_info_3seed_remaining.txt" <<EOF
family=no_shared_full_ef_full_ggn_actor_fvp_nosqrt_clip_size_sweep_3seed_transition
...
completed_prefix=clip0.25_and_clip0.5_use_seed_subset_0_1_2
remaining_clips=1.0,2.0
...
while [[ -f "$RUN_ROOT/clip025/ant/seed0.rc" ]] && [[ "$(cat "$RUN_ROOT/clip025/ant/seed0.rc")" == "RUNNING_RETRY" ]]; do
  sleep 60
done
if [[ ! -f "$RUN_ROOT/clip025/ant/seed0.rc" ]] || [[ "$(cat "$RUN_ROOT/clip025/ant/seed0.rc")" != 0 ]]; then
  overall_failed=1
fi
