...
PREV_ROOT=$ROOT/perf_runs/workstation92_detjc_fullEF_GGN_actor_fvp_nosqrt_clip_sweep_c025_c05_c1_c2_all7_5seed_10m_20260729
RUN_ROOT=${RUN_ROOT:-$ROOT/perf_runs/workstation92_detjc_fullEF_GGN_actor_fvp_nosqrt_clip_sweep_c4_c8_all7_3seed_10m_20260730}
...
while [[ ! -f "$PREV_ROOT/status" ]] || [[ "$(cat "$PREV_ROOT/status")" == "RUNNING" ]]; do
  sleep 300
done
...
family=no_shared_full_ef_full_ggn_actor_fvp_nosqrt_large_clip_followup_3seed
host=mingfei-workstation-92
physical_gpus=0,1
trainer=$TRAINER
trainer_sha256=$(sha256sum "$TRAINER" | awk '{print $1}')
config_clip4_sha256=$(sha256sum configs/rat_mlp_detjc_full_ef_ggn_actor_fvp_nosqrt_clip4.yaml | awk '{print $1}')
config_clip8_sha256=$(sha256sum configs/rat_mlp_detjc_full_ef_ggn_actor_fvp_nosqrt_clip8.yaml | awk '{print $1}')
launcher_sha256=$(sha256sum "$0" | awk '{print $1}')
predecessor=$PREV_ROOT
predecessor_status=$(cat "$PREV_ROOT/status")
clips=4.0,8.0
...
for item in 4:0 8:1; do
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PY" -u "$TRAINER" \
    ...
    > "$preflight_root/clip${label}.stdout" 2> "$preflight_root/clip${label}.stderr" &
  preflight_pids+=("$!")
  preflight_labels+=("$label")
done

preflight_failed=0
for i in "${!preflight_pids[@]}"; do
  rc=0
  wait "${preflight_pids[$i]}" || rc=$?
  echo "$rc" > "$preflight_root/clip${preflight_labels[$i]}.rc"
  if [[ $rc -ne 0 ]]; then
    preflight_failed=1
  fi
done
...
echo PASS > "$preflight_root/status"
