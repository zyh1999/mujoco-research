...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    actor_curvature_rows = int(getattr(algo_config, 'actor_curvature_subsample', 0) or pi_minibatch_size)
    critic_curvature_rows = int(getattr(algo_config, 'critic_curvature_subsample', 0) or v_minibatch_size)
    print(
        "Curvature rows per minibatch: "
        f"actor={min(actor_curvature_rows, pi_minibatch_size)} "
        f"critic={min(critic_curvature_rows, v_minibatch_size)}",
        flush=True,
    )
