...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    def CriticGN_Update(_obs, _ret):
        ...
        critic_free_rho_mode = str(
            getattr(algo_config, 'critic_subsample_free_rho', 'none') or 'none'
        ).lower()
        use_critic_free_rho = (
            critic_free_rho_mode == 'anchor'
            and critic_curvature_subsample > 0
            and _obs.shape[0] > critic_curvature_subsample
        )
        if use_critic_free_rho and critic_curvature_subsample != 255:
            raise ValueError('critic energy-free mode requires critic_curvature_subsample=255')
        ...
        with torch.no_grad():
            ...
            critic_free_rho_value = 0.0
            critic_anchor_samples = num_samples

            if use_critic_free_rho:
                if str(getattr(algo_config, 'fisher_kernel', 'exact')).lower() != 'exact':
                    raise NotImplementedError('critic energy-free mode requires fisher_kernel=exact')
                if str(getattr(algo_config, 'fisher_kernel_normalization', 'none')).lower() != 'none':
                    raise ValueError('critic energy-free mode requires normalization=none')

                full_m = int(J.shape[0])
                anchor_mask = torch.zeros(full_m, device=J.device, dtype=torch.bool)
                anchor_mask[critic_curvature_inds] = True
                nonanchor_inds = torch.arange(full_m, device=J.device)[~anchor_mask]
                J_s = J.index_select(0, critic_curvature_inds)
                J_n = J.index_select(0, nonanchor_inds)
                b_s = _residual.index_select(0, critic_curvature_inds).to(dtype=J.dtype)
                b_n = _residual.index_select(0, nonanchor_inds).to(dtype=J.dtype)
                rho_metric = torch.dot(b_n, b_n).clamp_min(
                    float(getattr(algo_config, 'critic_subsample_free_rho_eps', 1e-12))
                )
                rho_basis = J_n.t().mv(b_n)
                critic_basis = torch.cat([J_s, rho_basis.unsqueeze(0)], dim=0)
                rhs = torch.cat([b_s, rho_metric.unsqueeze(0)], dim=0)
                reduced_samples = critic_curvature_subsample + 1
                damping_metric = torch.ones_like(rhs)
                damping_metric[-1] = rho_metric
                reduced_system = critic_basis @ critic_basis.t() / float(reduced_samples)
                reduced_system = reduced_system + float(algo_config.cg_damping) * torch.diag(damping_metric)
                alpha = torch.linalg.solve(reduced_system, rhs)
                step_dir = critic_basis.t().matmul(alpha) / float(reduced_samples)
                critic_free_rho_value = float(alpha[-1].item())
                critic_anchor_samples = critic_curvature_subsample
                num_curvature_samples = reduced_samples
            else:
        ...
        with torch.no_grad():
            v_info = dict(v_grad_norm=grad_norm.item(), v_step_norm=step_dir.norm().item(),
                          critic_curvature_samples=num_curvature_samples,
                          critic_anchor_samples=critic_anchor_samples,
    ...
    for epoch in tepochs:
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if v_info.get('critic_free_rho', 0):
                logger.logkv("critic_anchor_samples", v_info['critic_anchor_samples'])
                logger.logkv("critic_free_rho", v_info['critic_free_rho'])
                logger.logkv("critic_free_rho_value", v_info['critic_free_rho_value'])
                logger.logkv("critic_independent_anchors", v_info['critic_independent_anchors'])
...
def train_fn(rank, world_size, algo, seed, algo_config, env_config, nets_config, log_config, device=-1):
    ...
    if rank==0:
        ...
        critic_rho_mode = str(getattr(algo_config, 'critic_subsample_free_rho', 'none') or 'none')
        if actor_rho_mode != 'none' or critic_rho_mode != 'none':
            subcurve_tag += f".arho_{actor_rho_mode}.crho_{critic_rho_mode}.independentanchors"
        if env_name in {'cartpole', 'acrobot', 'mountaincar', 'lunarlander', 'carracing', 'hopper', 'invertedpendulum', 'inverteddoublependulum',
                        'halfcheetah', 'walker2d', 'humanoid', 'humanoidstandup', 'reacher', 'swimmer', 'ant'}:
...
def main():
    ...
    parser.add_argument('--sgd_momentum', type=float, default=None,
                        help='Classic SGD momentum shared by the independent actor and critic optimizers.')
    ...
    parser.add_argument('--critic_subsample_free_rho', type=str, default=None, choices=['none', 'anchor'],
                        help='Add one residual-energy rho coordinate to an independently sampled critic anchor set.')
    parser.add_argument('--critic_subsample_free_rho_eps', type=float, default=None,
                        help='Numerical epsilon for the critic free-rho residual-energy metric.')
    ...
    if not hasattr(algo_config, 'critic_subsample_free_rho'):
        algo_config.critic_subsample_free_rho = 'none'
    if not hasattr(algo_config, 'critic_subsample_free_rho_eps'):
        algo_config.critic_subsample_free_rho_eps = 1e-12
    ...
    if args.critic_subsample_free_rho is not None:
        algo_config.critic_subsample_free_rho = args.critic_subsample_free_rho
    if args.critic_subsample_free_rho_eps is not None:
        algo_config.critic_subsample_free_rho_eps = args.critic_subsample_free_rho_eps
