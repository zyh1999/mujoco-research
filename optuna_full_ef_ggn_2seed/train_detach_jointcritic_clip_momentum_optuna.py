...
def learn(world_size, algo, actor_critic, writer, venv, device,
    ...
    if algo_config.optimizer == 'adam':
        ...
        actor_sgd_momentum = float(getattr(algo_config, 'actor_sgd_momentum', 0.0))
    ...
    if critic_optimizer_name == 'adam':
        ...
        critic_sgd_momentum = float(getattr(algo_config, 'critic_sgd_momentum', 0.0))
    ...
    if actor_clip_mode not in {'l2', 'fvp_fisher', 'fvp_fisher_sqrt'}:
    ...
    def RAT_ActorUpdate(_obs, _act, _adv, _outputs_old):
        ...
        if actor_clip_mode in {'fvp_fisher', 'fvp_fisher_sqrt'} and (
            actor_curvature_inds is not None
            or use_grouped_rho
            or use_free_rho
            or effective_full_gradient_rhs
        ):
            raise ValueError("FVP actor clipping currently requires full-batch Exact EF")

        # Update actor. The two FVP modes differ only in whether q is square-rooted.
        #   q = g^T (F + damping I) g
        ...
        if use_grouped_rho:
            ...
            pi_optimizer.zero_grad()
            _loss = _loss_pi - algo_config.ent_coef * _entropy
            if actor_clip_mode in {'fvp_fisher', 'fvp_fisher_sqrt'}:
                if str(getattr(algo_config, 'fisher_kernel', 'exact')).lower() != 'exact':
                ...
                clip_denominator = (
                    actor_fvp_quadratic_pre.sqrt()
                    if actor_clip_mode == 'fvp_fisher_sqrt'
                    else actor_fvp_quadratic_pre
                )
    ...
    for epoch in tepochs:
        ...
        if logger.get_dir() is not None and (epoch+1) % log_config.log_interval == 0:
            ...
            if pi_info.get('actor_clip_mode') in {'fvp_fisher', 'fvp_fisher_sqrt'}:
    ...
    if log_dir is not None:
        # save checkpoints
        if bool(getattr(algo_config, 'save_final_checkpoint', True)):
...
def main():
    ...
    parser.add_argument('--actor_clip_mode', type=str, default=None,
                        choices=['l2', 'fvp_fisher', 'fvp_fisher_sqrt'])
